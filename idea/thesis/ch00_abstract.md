...
- **C1（不確定性訊號審計）**：系統性量測哪些信心訊號對「答對與否」與「無目標」具有資訊量、哪些沒有。跨提示一致性（cross-prompt consistency）作為與定位相關的擾動訊號，在三個 split 上對答對與否的 AUROC 達約 0.72；OWL-ViT 的分數熵對無目標的 AUROC 達約 0.68。
