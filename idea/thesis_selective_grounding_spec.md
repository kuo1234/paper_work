...
舊計畫把 VIRO 寫成 *ICLR'26 withdrawn / unpublished*。**此描述已過期**：VIRO 已有 arXiv 與 CVPR 2026 版本，必須當作**已發表競品**對待，不能再用「withdrawn」當作弱化它的理由。

- 引用：`〔待補：VIRO 正式 arXiv ID + CVPR 2026 bibkey〕`（請填入後我再把 BibTeX key 接進差異表）。
...
| **VIRO**（arXiv / CVPR'26）| REC + abstention | 視其設定 | 是（含 no-target / verification） | **LLM program synthesis + neuro-symbolic 驗證 pipeline** | framing 最近；**方法路線正交**：VIRO 走程式合成 + 符號執行器逐步驗證，本研究走 frozen base + 輕量 post-hoc calibrator，無程式合成、無符號執行器、base-agnostic、近零訓練。兩者可互補對照 |
...
- **VIRO (CVPR'26)**：neuro-symbolic verification 的 abstention。列為**對照路線**，標註其 program synthesis 成本；若能取得其 no-target 數字則同表並列，取不到則只在文字 positioning。
...
## 待補清單
- `〔待補〕` VIRO 正式 arXiv ID + CVPR 2026 bibkey（填入後接進 §1.3 / §3.2）。
- HieA2G no-target 官方數字（若同表並列需查證頁碼）。
